variable "vars_to_store" {
  description = "List of regular (non-secret) vars to store in Clan vars, each with a string value and a list of target machines. Example: [{ name = 'debug_mode', value = 'true', machines = ['server1'] }, { name = 'debug_mode', value = 'false', machines = ['server2'] }]."
  type = list(object({
    name     = string # Added 'name' field to identify the variable
    value    = string
  ...
  validation {
    ...
    error_message = "Each var entry must have a 'name' (string), a 'value' (string), and 'machines' must be a non-empty list of strings."
  }
}
